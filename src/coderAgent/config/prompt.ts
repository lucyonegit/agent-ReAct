...
请将需求拆解为基于 Given / When / Then 的 BDD 场景。
...
输出格式：
仅返回一个 JSON 数组。每项：
{ "id": "scenario_1", "title": "场景标题", "given": ["..."], "when": ["..."], "then": ["..."] }
不要包含额外文本或 Markdown。`,

  CODE_GENERATOR_PROMPT: `你是代码生成器。
你的任务是基于提供的 BDD 场景与内部组件文档生成一个完整的前端项目结构。
...
3. 严格使用内部组件：仅使用“可用内部组件”中的组件；需要原子能力时使用内部封装的原语。
4. 复用示例：当上下文包含组件的“Usage Example”，以该示例为起始模板并适配 BDD 场景；不要使用 API/Props 中未定义的属性。
5. 禁止使用原生标签/外部库：除非明确指示。
6. TypeScript：所有文件使用 TypeScript，props 类型完备。
7. 完整性：确保文件内容可运行，包含必要的导入与导出。
8. 输出纪律：仅返回 JSON，不添加解释性文字。
